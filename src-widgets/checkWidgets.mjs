 ...
 * `widgets/canvas-gauges.html` - vis-2 replaces a widget type when a React widget declares the same id. And
...
const PREV_PREFIX = 'widgets/vis-2-widgets-canvas-gauges/';
 ...
 *
 * `highlightsFrom` and friends are the fields of the indexed group: vis-1 wrote them as `highlightsFrom(1-hCount)`
 * and stores them as `highlightsFrom1`, `highlightsFrom2`, ... - here the group carries `indexFrom`/`indexTo` and
 * the field keeps the bare name, so the stored names are the same.
 */
const DROPPED = {};
...
// `canvas-gauges` draws into a canvas and touches `document` while loading - it never runs here, so a stub of
// the two classes is enough and keeps this check free of a DOM
const gaugeStub = path.join(TMP, 'canvas-gauges.ts');
writeFileSync(
    gaugeStub,
...
await build({
    configFile: false,
    logLevel: 'error',
    resolve: { alias: { 'canvas-gauges': gaugeStub } },
    build: {
 ...
 *
 * `<script id=` and not just `id=`: an older, shorter version of `tplCGlinearGauge` stands commented out above
 * the real one (`<!--script id=...`), and it does not declare the groups of the bar.
 */
function legacyAttrs(tpl) {
    const start = legacyHtml.indexOf(`<script id="${tpl}"`);
    if (start === -1) {
        return null;
    }
